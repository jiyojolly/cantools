...
def _do_decode(args):
    ...
    print("Converting to UTC time stamps")
    dict_Mat = {}
    for key,value in plotter.signals.values.items():
        utc_timestamps = [] 
        for time_stamp in value.x:
            utc_timestamps.append(time_stamp.replace(tzinfo=datetime.timezone.utc).timestamp())

        dict_Mat[key.replace('.','_')] = (utc_timestamps, value.y)
    
    print("Exporting to MAT file...")
    savemat("CAN_Signals.mat", dict_Mat)
